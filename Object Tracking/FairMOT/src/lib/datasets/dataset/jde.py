...
class LoadImages:  # for inference
    def __init__(self, path, img_size=(1088, 608)):
        if os.path.isdir(path):
            ...
            li = os.listdir(path)
            li = [int(e[:-4]) for e in li]
            li.sort()
            li = [str(e)+'.jpg' for e in li]
            li = [os.path.join(path, file) for file in li]
            self.files = list(filter(lambda x: os.path.splitext(x)[1].lower() in image_format, li))
